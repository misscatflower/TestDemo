package com.codey.controller;

import com.codey.util.PrintSH;
import lombok.extern.slf4j.Slf4j;
import org.springframework.web.bind.annotation.*;
import com.codey.util.GETTime;


@Slf4j
@RestController
@RequestMapping("/jacoco3")
public class Test3Controller {
    GETTime getTime = new GETTime();

    @RequestMapping(value = "addTest1", method = RequestMethod.GET)
    public String addTest1(String userName, String password) {
        log.info("--------------------方法进入addTest1");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty() || password.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空test";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功test";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改test");
            return "登录也成功test";
        } else if ("999".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改test");
            return "999登录也成功test";
        } else {
            printSH.throw_exception1(2);
            return "登录失败test";
        }
    }

    @RequestMapping(value = "addTest2", method = RequestMethod.GET)
    public String addTest2(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有 效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "addTest3", method = RequestMethod.GET)
    public String addTest3(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "addTest4", method = RequestMethod.GET)
    public String addTest4(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "newTest1", method = RequestMethod.GET)
    public String newTest1(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "newTest2", method = RequestMethod.GET)
    public String newTest2(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "getTest1", method = RequestMethod.GET)
    public String getTest1(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "getTest2", method = RequestMethod.GET)
    public String getTest2(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("修改的方法");
        System.out.println("修改了，测试增量覆盖率");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            return "登录也成功";
        } else if ("999".equals(password)) {
            getTime.getTime(3);
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "getTest3", method = RequestMethod.GET)
    public String getTest3(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("修改的方法");
        System.out.println("修改了，测试增量覆盖率");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            return "登录也成功";
        } else if ("999".equals(password)) {
            getTime.getTime(3);
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "getTest4", method = RequestMethod.GET)
    public String getTest4(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("修改的方法");
        System.out.println("修改了，测试增量覆盖率");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            return "登录也成功";
        } else if ("999".equals(password)) {
            getTime.getTime(3);
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }

    @RequestMapping(value = "newTest111", method = RequestMethod.GET)
    public String newTest111(String userName, String password) {
        log.info("--------------------方法进入");
        PrintSH printSH = new PrintSH();
        printSH.printLog1();
        printSH.printLog1("打印的字符串printLog1");
        if (userName.isEmpty()) {
            printSH.throw_exception1(1);
            return "登录失败，姓名不能为空";
        }
        // 方法中的注释
        if ("888".equals(password)) {
            getTime.getTime(2);
            // 分支注释
            return "888登录成功";
        } else if ("777".equals(password)) {
            getTime.getTime(3);
            System.out.print("分支有效代码修改");
            return "登录也成功";
        } else {
            printSH.throw_exception1(2);
            return "登录失败";
        }
    }
}
